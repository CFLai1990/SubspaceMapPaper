...
The analysis of subspaces of a complex high-dimensional dataset is extremely challenging. On the one hand, the number of subspaces grows exponentially as the dimensionality increases. On the other hand, the data distributions could be highly different across various subspaces. 
In this paper, we propose S-Map (i.e. the Subspace Map), a novel visualization approach with the map metaphor to summarize the features of subspaces and guide users for efficient explorations. In S-Map, subspaces are clustered into 'continents' and 'islands' depending on their population and similarities. Altitudes of the lands reflect the densities of the clusters. For each clustered region, we extract its featured dimensions and stable structures to reveal the impacts of dimension combinations to the data distributions. On different clustering levels, we pick out the representative subspaces as 'capital cities' and build up a traffic system between them to guide users in the tour. Case studies with real-world datasets are presented to demonstrate the effectiveness of the proposed system.
} % end of abstract
